...
type TheaterDetailClientProps = {
  theater: Theater;
};

export default function TheaterDetailClient({
  theater,
}: TheaterDetailClientProps) {
  ...
  useEffect(() => {
    ...
    const checkFavoriteStatus = async () => {
      ...
      try {
        const response = await fetch(
          `/api/favorites/check?theaterId=${theater.id}`
        );
        if (response.ok) {
    ...
    checkFavoriteStatus();
  }, [session, theater.id, searchParams]);
